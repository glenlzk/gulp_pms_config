;(function (window, document, $, yzl) {
	var totalOrder = new Vue({
        ...
        data: {
            ...
            selectText: '搜索请输入...',
                // multi select
            isMultiple: true,
            totalChannelElemId: 'totalChannelElemId',
                // default selected
            isSelected: true,
            // select vue end
            initdata:{
            ...
            //权限管理
            totalOrderResourceInfo: JSON.parse(sessionStorage.getItem("Cookie_headerResourceInfo"))
        },
        // 分页跳转监听
        watch: {
		...
		methods: {
            ...
            initData: function () {
                ...
                if (_self.totalChannelList.length == _self.channleCodelist.length) {
            ...
            initChannel: function(){
            	...
            	yzl.getAjax({
                    ...
                    sCallback : function (result) {
                        if (result.code == "0000") {
                            _self.totalChannelList = result.data.channelForQueryList;
    ...
    totalOrder.initEvent();

    // 订单详情修改后，需要刷新uncheckinOrder的initData
    yzl.contentVue.initPageData = totalOrder.initData;
