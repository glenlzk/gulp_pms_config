;(function (window, document, $, yzl) {
	var hascheckinOrder = new Vue({
        ...
        data: {
            ...
            // multi select
            isMultiple: true,
            hascheckinChannelElemId: 'hascheckinChannelElemId',
            // default selected
            isSelected: true,
            // select vue end
            initdata:{
            ...
            //权限管理
            hascheckinOrderResourceInfo : JSON.parse(sessionStorage.getItem("Cookie_headerResourceInfo"))
        },
        // 分页跳转监听
        watch: {
		...
		methods: {
            ...
            initChannel: function(){
            	...
            	yzl.getAjax({
                    ...
                    sCallback : function (result) {
                        if (result.code == "0000") {
                            _self.hascheckinChannelList = result.data.channelForQueryList;
	    ...
	    hascheckinOrder.initEvent();

        // 订单详情修改后，需要刷新hascheckinOrder的initData
        yzl.contentVue.initPageData = hascheckinOrder.initData;
